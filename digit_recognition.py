...
#     mini_batches = [train_set[x:x + batch_size] for x in range(0, number_of_samples, batch_size)]
...
#             for j in range(10):
#                 for k in range(16):
#                     third_grad_W[j, k] += 2*(nn_outputs[j, 0]-nn_outputs_star[j, 0])*(nn_outputs[j, 0]*(1-nn_outputs[j, 0]))*second_layer_output[k, 0]
#                 third_grad_b[j, 0] += 2*(nn_outputs[j, 0]-nn_outputs_star[j, 0])*(nn_outputs[j, 0]*(1-nn_outputs[j, 0]))
#             #calculate dcost/d(second_layer_output)
#             for k in range(16):
#                 for j in range(10):
#                     d_cost_d_second_layer_output[k] += 2*(nn_outputs[j, 0]-nn_outputs_star[j, 0])*(nn_outputs[j,0]*(1-nn_outputs[j, 0]))*third_layer_weight[j, k]
#             #second layer
#             # update second_grad_W (dcost/d(second_layer_weight)) and second_grad_b (dcost/d(second_layer_bias))
#             for k in range(16):
#                 for m in range(16):
#                     second_grad_W[k, m] += d_cost_d_second_layer_output[k]*(second_layer_output[k, 0]*(1-second_layer_output[k, 0]))*first_Layer_output[m, 0]
#                 second_grad_b[k, 0] += d_cost_d_second_layer_output[k]*(second_layer_output[k, 0]*(1-second_layer_output[k, 0]))
#             for m in range(16):
#                 for k in range(16):
#                     d_cost_d_first_layer_output[m] += d_cost_d_second_layer_output[k]*(second_layer_output[k, 0]*(1-second_layer_output[k, 0]))*second_layer_weight[k, m]
#             #firs layer
#             for m in range(16):
#                 for v in range(784):
#                     first_grad_W[m, v] += d_cost_d_first_layer_output[m]*(first_Layer_output[m, 0]*(1-first_Layer_output[m, 0]))*nn_inputs[v, 0]
#                 first_grad_b[m, 0] += d_cost_d_first_layer_output[m]*(first_Layer_output[m, 0]*(1-first_Layer_output[m, 0]))
#
...
# end = time.time()
#
# #check the accuracy of nn with learning step3
# correct_output_step3 = 0
# for i in range(100):
#     nn_inputs = train_set[i][0]
#     first_Layer_output = sig(first_layer_weight @ nn_inputs + first_layer_bias)
#     second_layer_output = sig(second_layer_weight @ first_Layer_output + second_layer_bias)
...
#print the result
t = (end-start)/60
#print("The Accuracy of step 3: ", correct_output_step3/100)
#print("The Accuracy of step 4: " , correct_output_step4/100)
...
print("Duration of learning (minutes): ", end="")
print ("{0:.2f}".format(t))
